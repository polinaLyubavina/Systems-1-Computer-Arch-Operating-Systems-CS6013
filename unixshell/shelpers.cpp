      ...
      for( int j = first + 1; j < last; ++j ) {
         ...
         if( tokens[j] == ">" || tokens[j] == "<" ) {
            ...
            // Only the LAST command can have output redirection!
             ...
             if (tokens[j] == "<") {
                 commands[j].inputFd = open(tokens[j+1].c_str(), O_RDONLY);
               if (commands[j].inputFd < 0)
                 perror("error opening file\n");
             }
             // output file, setting stdout to name of file
             if (tokens[j] == ">") {
                 commands[j].outputFd = open(tokens[j+1].c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
               if (commands[j].outputFd < 0)
                 perror("error writing to file\n");
               // add nullptr to signal to exec end of args
                 commands[j].argv.push_back(nullptr);
             }
         }
         else if( tokens[j] == "&" ){
            // Fill this in if you choose to do the optional "background command" part.
//            assert(false);
             commands[j].background = true;
         }
         else {
      ...
      if( !error ) {
         ...
         if( cmdNumber > 0 ){
             ...
             commands[p].inputFd = fds[0];
             commands[p - 1].outputFd = fds[1];
         }
   ...
   if( error ){
      ...
      // has not gotten to it when the error (in a previous command) occurred.
       ...
       for(Command command : commands) {
           ...
           else if(command.outputFd != 1) {
               close(command.outputFd);
           }
       }
       std::cout << "Error in getCommands(), could not process" << std::endl;
       exit(1);
//      assert(false);
   }
